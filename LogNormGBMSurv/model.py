...
class LogNormGBMSurvivalModel:
    ...
    survival distribution — specifically the Log-Normal distribution with `mu` and `sigma`
    ...
    def fit(self, X, y):
        ...
        if self.initial_params:
            ...
            init_value_mu = np.log(mean) - 0.5 * np.log(var / (mean ** 2) + 1)
    ...
    def predict(self, X):
        ...
        predicted_params[:, 1] = np.maximum(predicted_params[:, 1], 10e-3)

        mu = predicted_params[:, 0][:, np.newaxis]
        sigma = predicted_params[:, 1][:, np.newaxis]

        t_i = np.array(self._times)
        t_next = np.append(t_i[1:], np.inf)

        laplas_func_t_next = norm.cdf(np.log(t_next), loc=mu, scale=sigma)
        laplas_func_t_i = norm.cdf(np.log(t_i), loc=mu, scale=sigma)

        predicted_proba = laplas_func_t_next - laplas_func_t_i
