...
def prepare_data_for_computing(preds: np.ndarray, dtrain: xgb.DMatrix):
    ...
    valid_right_indices = np.minimum(
        time_indices[~mask] + 1 + k, len(time_intervals) - 1)
    ...
    phi_values_right = norm.pdf(
        log_time_right, loc=mu[~mask], scale=sigma[~mask])
    phi_values_left = norm.pdf(
        log_time_left, loc=mu[~mask], scale=sigma[~mask])

    laplas_func_values_right = norm.cdf(
        log_time_right, loc=mu[~mask], scale=sigma[~mask])
    laplas_func_values_left = norm.cdf(
        log_time_left, loc=mu[~mask], scale=sigma[~mask])
    ...
    phi_values_left_masked = norm.pdf(
        log_time_left_masked, loc=mu[mask], scale=sigma[mask])
    laplas_func_values_left_masked = norm.cdf(
        log_time_left_masked, loc=mu[mask], scale=sigma[mask])
...
def surv_grad_hess(preds: np.ndarray,
    ...
    shape, censoring, mask, sigma, log_t_mu_diff_right, phi_values_right, laplas_func_values_right, log_t_mu_diff_left, phi_values_left, laplas_func_values_left, log_t_mu_diff_left_masked, phi_values_left_masked, laplas_func_values_left_masked = prepare_data_for_computing(
        preds, dtrain)
    ...
    laplas_func_diff = np.maximum(
        laplas_func_values_right - laplas_func_values_left, 10e-20)

    laplas_func_one_diff_right = np.maximum(
        1 - laplas_func_values_right, 10e-20)
    laplas_func_one_diff_left_masked = np.maximum(
        1 - laplas_func_values_left_masked, 10e-20)

    # Calculate gradients for all but the last interval
    grad[~mask, 0] += censoring[~mask] * \
        (phi_values_right - phi_values_left) / (laplas_func_diff)
    grad[~mask, 1] += censoring[~mask] * ((log_t_mu_diff_right / sigma[~mask]) * phi_values_right - (
        log_t_mu_diff_left / sigma[~mask]) * phi_values_left) / (laplas_func_diff)

    grad[~mask, 0] -= (1 - censoring[~mask]) * \
        phi_values_right / laplas_func_one_diff_right
    grad[~mask, 1] -= (1 - censoring[~mask]) * (log_t_mu_diff_right /
                                                sigma[~mask]) * phi_values_right / laplas_func_one_diff_right

    # Handle the last interval separately
    grad[mask, 0] -= censoring[mask] * \
        phi_values_left_masked / laplas_func_one_diff_left_masked
    grad[mask, 1] -= censoring[mask] * (log_t_mu_diff_left_masked / sigma[mask]) * \
        phi_values_left_masked / laplas_func_one_diff_left_masked
    ...
    v = ((log_t_mu_diff_right / sigma[~mask]) * phi_values_right -
         (log_t_mu_diff_left / sigma[~mask]) * phi_values_left)

    u_derivative = v / (u ** 2)
    v_derivative = (log_t_mu_diff_right / (sigma[~mask] ** 2)) * phi_values_right * ((log_t_mu_diff_right / sigma[~mask]) ** 2 - 2) - (
        log_t_mu_diff_left / (sigma[~mask] ** 2)) * phi_values_left * ((log_t_mu_diff_left / sigma[~mask]) ** 2 - 2)
    ...
    hess[~mask, 1] -= (1 - censoring[~mask]) * (log_t_mu_diff_right ** 3 / sigma[~mask] ** 4 * phi_values_right / laplas_func_one_diff_right
                                                - 2 * log_t_mu_diff_right /
                                                sigma[~mask] ** 2 * phi_values_right /
                                                laplas_func_one_diff_right
    ...
    hess[mask, 1] -= censoring[mask] * (log_t_mu_diff_left_masked ** 3 / sigma[mask] ** 4 * phi_values_left_masked / laplas_func_one_diff_left_masked
                                        - 2 * log_t_mu_diff_left_masked /
                                        sigma[mask] ** 2 * phi_values_left_masked /
                                        laplas_func_one_diff_left_masked
...
def surv_loss(preds: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[str, float]:
    ...
    shape, censoring, mask, _, _, _, laplas_func_values_right, _, _, laplas_func_values_left, _, _, laplas_func_values_left_masked = prepare_data_for_computing(
        preds, dtrain)
    ...
    loss -= np.sum(censoring[~mask] *
                   safe_log(laplas_func_values_right - laplas_func_values_left))
    loss -= np.sum((1 - censoring[~mask]) *
                   safe_log(1 - laplas_func_values_right))
    loss -= np.sum(censoring[mask] * safe_log(1 -
                   laplas_func_values_left_masked))
