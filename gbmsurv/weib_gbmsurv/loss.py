...
def prepare_data_for_computing(preds: np.ndarray, dtrain: xgb.DMatrix):
    ...
    k_intervals = 0  # increase interval to lenght of 2 * k_intervals + 1

    valid_left_indices = np.maximum(time_indices[~mask] - k_intervals, 0)
    valid_right_indices = np.minimum(
        time_indices[~mask] + 1 + k_intervals, len(time_intervals) - 1)
...
def surv_grad_hess(preds: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[np.ndarray, np.ndarray]:
    ...
    shape, censoring, mask, lamb, k, time_ratio_left_pow_k, time_ratio_right_pow_k, exp_diff_replacement, time_ratio_left, time_ratio_right, time_ratio_left_masked, time_ratio_left_pow_k_masked = prepare_data_for_computing(
        preds, dtrain)
    ...
    grad[~mask, 0] -= (1 - censoring[~mask]) * (k[~mask] /
                                                lamb[~mask]) * time_ratio_right_pow_k
    ...
    grad[~mask, 1] += (1 - censoring[~mask]) * \
        time_ratio_right_pow_k * safe_log(time_ratio_right)

    # Handle the last interval separately
    grad[mask, 0] -= censoring[mask] * \
        (k[mask] / lamb[mask]) * time_ratio_left_pow_k_masked
    grad[mask, 1] += censoring[mask] * \
        time_ratio_left_pow_k_masked * safe_log(time_ratio_left_masked)
    ...
    u_lambda = k[~mask] * (np.exp(-time_ratio_left_pow_k) * time_ratio_left_pow_k -
    ...
    hess[~mask, 0] -= censoring[~mask] * \
        ((u_derivative_lambda * v_lambda -
         u_lambda * v_derivative_lambda) / (v_lambda ** 2))
    hess[~mask, 0] += (1 - censoring[~mask]) * k[~mask] * \
        (k[~mask] + 1) * time_ratio_right_pow_k / (lamb[~mask] ** 2)
    ...
    u_derivative_k = (np.exp(-time_ratio_right_pow_k) * time_ratio_right_pow_k * (safe_log(time_ratio_right) ** 2)
                      - np.exp(-time_ratio_right_pow_k) * (time_ratio_right_pow_k **
                                                           2) * (safe_log(time_ratio_right) ** 2)
                      - np.exp(-time_ratio_left_pow_k) *
                      time_ratio_left_pow_k * (safe_log(time_ratio_left) ** 2)
    ...
    hess[~mask, 1] -= censoring[~mask] * \
        ((u_derivative_k * v_k - u_k * v_derivative_k) / (v_k ** 2))
    hess[~mask, 1] += (1 - censoring[~mask]) * \
        time_ratio_right_pow_k * (safe_log(time_ratio_right) ** 2)

    # Handle the last interval separately
    hess[mask, 0] += censoring[mask] * k[mask] * \
        (k[mask] + 1) * time_ratio_left_pow_k_masked / (lamb[mask] ** 2)
    hess[mask, 1] += censoring[mask] * time_ratio_left_pow_k_masked * \
        (safe_log(time_ratio_left_masked) ** 2)
...
def surv_loss(preds: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[str, float]:
    ...
    shape, censoring, mask, _, _, _, time_ratio_right_pow_k, exp_diff_replacement, _, _, _, time_ratio_left_pow_k_masked = prepare_data_for_computing(
        preds, dtrain)
    ...
    loss -= np.sum(censoring[mask] *
                   safe_log(np.exp(-time_ratio_left_pow_k_masked)))
