...
def surv_grad_hess(preds: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[np.ndarray, np.ndarray]:
    ...
    mask_i_is_not_last_class = indices_last_class != correct_class.reshape(-1, 1)
    mask_less_equal_i = (indices <= correct_class.reshape(-1, 1)) == mask_i_is_not_last_class
    ...
    grad_0[mask_more_i] = (s * (sum_exp_l/sum_exp_r_nonzero)[:, None])[mask_more_i]
    ...
    hess_0[mask_less_equal_i] = - s[mask_less_equal_i] * (1 - s[mask_less_equal_i])
    hess_0[mask_more_i] = (s * (1 - s) * (sum_exp_l/sum_exp_r_nonzero)[:, None])[mask_more_i] + (s * (-1) * exp_preds * (sum_exp_l / (sum_exp_r_nonzero ** 2))[:, None])[mask_more_i]
...
def surv_loss(preds: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[str, float]:
    ...
    mask_after_correct_class = np.tile(np.arange(C), (N, 1)) > correct_class[:, None]

    loss -= np.sum(censoring * np.log(s[mask_s_correct_class]))
    
    sum_s_after_correct_class = np.sum(s * mask_after_correct_class, axis=1)
    sum_s_after_correct_class_nonzero = np.where(sum_s_after_correct_class == 0, 1e-0, sum_s_after_correct_class)
